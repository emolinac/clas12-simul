...
sub build_targets
{
	...
	# target container
	# and inside cell
	if($thisVariation eq "lH2" || $thisVariation eq "lD2" || $thisVariation eq "lH2e" || $thisVariation eq "3cmlD2")
	{
		my $nplanes = 4;
		...
		if ($thisVariation eq "3cmlD2") {
			@oradiusT  =  (   2.5,  0.9 * 10.3,  7.3, 5.0,  2.5);
			@z_planeT  =  ( -14.2, -11.2, 12.5, 13.5, 14.5);
		}
		...
		$detector{"dimensions"}  = $dimen;
		$detector{"material"}    = "G4_lH2";
		if($thisVariation eq "lD2" || $thisVariation eq "3cmlD2") {
			$detector{"material"}    = "LD2";
		}
		$detector{"style"}       = 1;
		print_det(\%configuration, \%detector);
