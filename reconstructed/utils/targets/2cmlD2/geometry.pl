...
sub build_targets
{
	...
	# target container
	# and inside cell
	if($thisVariation eq "lH2" || $thisVariation eq "lD2" || $thisVariation eq "lH2e" || $thisVariation eq "2cmlD2")
	{
		my $nplanes = 4;
		...
		if ($thisVariation eq "2cmlD2") {
			@oradiusT  =  (   2.5,  0.8 * 10.3,  7.3, 5.0,  2.5);
			@z_planeT  =  ( -9.2, -6.2, 7.5, 8.5, 9.5);
		}
		# actual target
		%detector = init_det();
		$detector{"name"}        = "lh2";
		...
		$detector{"dimensions"}  = $dimen;
		$detector{"material"}    = "G4_lH2";
		if($thisVariation eq "lD2" || $thisVariation eq "2cmlD2") {
			$detector{"material"}    = "LD2";
		}
		$detector{"style"}       = 1;
		print_det(\%configuration, \%detector);
